...
![Status](https://img.shields.io/badge/status-V1%20Toy%20DQN%20smoke-2E8B57)
![Test](https://img.shields.io/badge/tests-pytest%20passing-4C9A2A)
![RL](https://img.shields.io/badge/RL-pure%20Python%20DQN-F28C28)
...
V0는 실제 서울 전체 따릉이 운영을 재현하지 않는다. 대신 대여소 3개와 재배치 트럭 1대를 사용하는 toy environment를 만들고, `State`, `Action`, `Reward`, `Transition`이 명확한 최소 MDP를 먼저 검증한다. 현재 저장소는 V0 baseline을 기준선으로 고정하고, 같은 MDP 위에서 V1 DQN 학습과 greedy 평가를 실행할 수 있는 단계다.

> [!IMPORTANT]
> V0에서 DQN이 학습할 대상은 “자전거를 몇 대 옮길지”가 아니라 **다음에 어느 대여소를 방문할지**이다. 싣기/내리기 수량은 환경의 rule-based heuristic이 처리한다.

> [!NOTE]
> 현재 V1 DQN은 PyTorch 모델이 아니라, 외부 딥러닝 의존성 없이 구현한 **순수 Python one-hidden-layer Q-network smoke prototype**이다. PyTorch 기반 DQN/Double DQN은 다음 확장 단계로 둔다.
...
> 작은 toy MDP에서 DQN이 시간대별 수요 패턴과 현재 재고를 보고, Random 및 Low-stock heuristic보다 평균 헛걸음을 줄이면서 이동비용을 관리하는 방문 순서 정책을 학습할 수 있는가?
...
이 프로젝트에서 DQN은 이미지 픽셀이 아니라 정규화된 재고/시간/트럭 상태를 입력으로 받고, 가능한 action 중 다음 방문 대여소를 선택하는 Q-network를 사용한다. 현재 구현은 순수 Python prototype이며, 학술적으로는 DQN의 핵심 구성인 Q-network, replay buffer, target network, epsilon-greedy exploration을 작은 toy MDP에서 검증하는 단계다.
...
| Environment | `ToyDdareungiEnv` | 트럭 이동, 자동 재배치, 수요/반납, reward 계산을 처리한다. |
...
| DQN | Q-network로 `Q(s, a)`를 학습해 action을 선택한다. | V1 학습 기반 정책 |
...
terminal step에서는 bootstrap term을 제거한다. V1에서는 replay buffer, target network, epsilon-greedy exploration을 사용해 방문 순서 정책을 학습하고, 학습 후에는 baseline과 동일한 평가 조건에서 비교한다.
...
주요 성공 기준은 두 baseline보다 낮은 `Avg Unmet Demand`와 높은 `Service Rate`다. `Avg Reward`는 unmet demand와 movement cost를 함께 반영하는 보조 종합 지표로 사용한다.
...
현재 V0 예시 결과에서는 Low-stock heuristic이 Random보다 낮은 성능을 보인다. 이는 단순히 현재 재고가 낮은 대여소를 방문하는 규칙이 시간대별 수요와 트럭 적재 상태를 충분히 반영하지 못할 수 있음을 보여준다. 이 관찰은 학습 기반 방문 정책을 비교할 필요성을 만든다.
...
| DQN 초기 단계 | 순수 Python MLP DQN으로 smoke 학습/평가 가능 | multi-seed 평가와 reward ablation |
...
| V1 | DQN이 baseline보다 나은 방문 순서 정책을 학습하는가? | DQN train/evaluate, greedy evaluation | 초기 구현 |
| V1.5 | 결과가 seed와 reward 설계에 안정적인가? | multi-seed evaluation, reward ablation | 예정 |
| V2 | DQN 변형이 안정성을 개선하는가? | Double DQN, Dueling DQN 비교 | 예정 |
| V3 | reward/dynamics를 더 현실적으로 만들 수 있는가? | 거리 기반 이동비용, 반납 실패 reward | 예정 |
| V4 | toy demand를 실제 데이터 패턴으로 대체할 수 있는가? | 실제 따릉이 데이터 일부 반영 | 예정 |
...
메뉴에서는 baseline 평가, DQN(Small) 학습, 저장된 DQN greedy 평가, visualization 포함 평가를 선택할 수 있다. 4번/5번 visualization 메뉴는 창을 닫으면 프로그램도 함께 종료된다.
...
짧은 학습 결과는 smoke check이며, 성능 주장은 held-out seed 묶음에서 Random, Low-stock, DQN을 같은 조건으로 비교한 뒤에만 한다.
