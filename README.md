...
실제 따릉이 CSV는 현재 MDP의 `State`, `Action`, `Reward` 정의를 바꾸지 않고, 시간대별 `demand/return` sampling profile을 구성하는 데만 사용한다. 즉, 이 단계는 서울 전체 운영 최적화가 아니라 **마곡 3개 대여소의 실제 거래 패턴을 반영한 small MDP 확장**이다.

현재 1차 profile 후보는 다음과 같다.

| 역할 | Station ID | 대여소명 | 의미 |
|---|---|---|---|
| 중심 허브 | `ST-2031` | 마곡나루역 2번 출구 | 활동량이 큰 기준 station |
| 반납-heavy | `ST-2033` | LG유플러스 마곡사옥 | 반납이 상대적으로 많은 station |
| 대여-heavy | `ST-2049` | 마곡수명산 1-2단지 | 대여가 상대적으로 많은 station |

주의할 점은 CSV의 대여/반납 기록이 실제 발생한 거래라는 것이다. 자전거가 없어서 대여하지 못한 잠재 수요는 CSV에 직접 기록되지 않을 수 있으므로, 이 데이터는 “수요 예측 정답”이 아니라 toy transition을 더 현실적으로 만드는 관측 profile로 해석한다.
...
| V5 | toy demand를 실제 데이터 패턴으로 대체할 수 있는가? | 실제 따릉이 데이터 일부 반영 | 1차 profile 파이프라인 구현 |
