...
V0는 실제 서울 전체 따릉이 운영을 재현하지 않는다. 대신 대여소 3개와 재배치 트럭 1대를 사용하는 toy environment를 만들고, `State`, `Action`, `Reward`, `Transition`이 명확한 최소 MDP를 먼저 검증한다. 현재 단계는 DQN 학습 전이며, Random baseline과 Low-stock heuristic baseline을 구현해 이후 DQN 성능을 비교할 기준선을 만든다.
...
| V1 | DQN이 baseline보다 나은 방문 순서 정책을 학습하는가? | DQN train/evaluate, greedy evaluation | 예정 |
...
| `evaluate.py` | baseline episode 실행, metric 집계, episode log 저장 |
