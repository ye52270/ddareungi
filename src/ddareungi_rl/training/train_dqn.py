...
@dataclass
class TrainingEpisodeMetric:
    """DQN 학습 episode 하나의 요약 지표를 저장한다."""
...
def train_dqn(
    ...
    global_step = 0
    metrics: list[TrainingEpisodeMetric] = []
    ...
    for episode in range(episodes):
        ...
        while not done:
            ...
            info["policy_name"] = "dqn"
            info["learning_stage"] = "DQN 학습 중"
        ...
        total_demand = total_served + total_unmet
        episode_metric = TrainingEpisodeMetric(
            episode=episode + 1,
...
def save_metrics(metrics: list[TrainingEpisodeMetric], output_path: Path) -> None:
    """학습 episode metrics를 JSON 파일로 저장한다."""
...
def parse_args() -> argparse.Namespace:
    """DQN 학습용 command-line argument를 파싱한다."""
    parser = argparse.ArgumentParser(description="Train a V1 DQN policy on ToyDdareungiEnv.")
    ...
    parser.add_argument(
        "--metrics-out",
        type=Path,
        default=Path("outputs/metrics/dqn_train_metrics.json"),
    )
